...
def init_pipeline():
    ...
    mm.load_models([
        "./part3_streaming_weights/diffusion_pytorch_model_streaming_dmd.safetensors",
    ])
    ...
    LQ_proj_in_path = "./part3_streaming_weights/LQ_proj_in.ckpt"
    ...
    mis = pipe.TCDecoder.load_state_dict(torch.load("./part3_streaming_weights/TCDecoder.ckpt"), strict=False)
    print(mis)
...
def main():
    ...
    for p in inputs:
        ...
        save_video(video, os.path.join(RESULT_ROOT, f"sr_infer_v11_tiny_long_{name.split('.')[0]}_seed{seed}.mp4"), fps=fps, quality=5)
